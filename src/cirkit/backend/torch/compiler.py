...
class TorchCompiler(AbstractCompiler):
    ...
    def _post_process_circuit(self, cc: AbstractTorchCircuit) -> AbstractTorchCircuit:
        if self.is_optimize_enabled:
            ...
            opt_cc = _optimize_circuit(self, cc, max_opt_steps=5, nystrom=self.is_nystrom_enabled)
            del cc
            cc = opt_cc
...
def _optimize_layers(
    ...
    *,
    ...
    if optimize_result is None:
        if nystrom and shatter and NystromPattern in patterns:
            raise ValueError("No layers matched for Nystr\xf6m optimization")
        return cc, False
