...
def plot_density_comparison_return(
    ...
    for idx, circuit in enumerate(trained_circuits):
        ...
        p_vals /= p_vals.sum() #TODO: CHECK THIS IS A LEGAL MANOEUVRE (vs logsumexp)
...
default_config = dict(
    ...
    # Model
    num_input_units = 1,      # will be overridden by the sweep
    num_sum_units   = 1,      # ditto
    sum_product     = "tucker",   # {cp, tucker}
...
def main():
    wandb.init(project="msc-cirkit", config=default_config)
    cfg = wandb.config
    print(cfg)
